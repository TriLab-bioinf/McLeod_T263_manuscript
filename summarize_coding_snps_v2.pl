...
my $usage = "$0 -i <vcf file> -f <alt_allele_freq [0.5]> -p T/F [print only high quality alleles] -m T/F [print missense alleles only] -P <ploidy [1]> -n <gene_name_file>\n\n";
my %arg = @ARGV;
die $usage unless ($arg{-i} and $arg{-p} and $arg{-m});
my %names;
my $ALLELE_FREQUENCY = $arg{-f} || 0.5; ## minimum allele frequency to call variants in each sample
if ($arg{-n}){
	open (NAME,"<$arg{-n}");
	while(<NAME>){
		chomp;
